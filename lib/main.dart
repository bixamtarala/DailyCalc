            ...
            (t) => Card(
              child: ListTile(
                ...
                onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => CalculatorScreen(tool: t.name),
                  ),
                ),
              ),
            ),
...
class _CalculatorScreenState extends State<CalculatorScreen> {
  ...
  List<String> get labels {
    switch (widget.tool) {
      ...
      case 'Compound Interest':
        return ['Principal (₹)', 'Annual rate (%)', 'Years'];
      case 'EMI Calculator':
        return ['Loan amount (₹)', 'Annual rate (%)', 'Months'];
      case 'Monthly Vaddi':
        return ['Principal (₹)', '₹ interest per ₹100/month', 'Months'];
      case 'Discount':
        return ['Price (₹)', 'Discount (%)', ''];
      case 'GST':
        return ['Amount (₹)', 'GST rate (%)', ''];
      case 'Fuel Cost':
        return ['Monthly distance (km)', 'Mileage (km/l)', 'Fuel price (₹/l)'];
      default:
        return ['Power (watts)', 'Hours/day', 'Days'];
    }
  }
  ...
  void calculate() {
    ...
    switch (widget.tool) {
      case 'Simple Interest':
        final i = CalculatorService.simpleInterest(x, y, z);
        r = 'Interest: ${money(i)}\nTotal: ${money(x + i)}';
        break;
      case 'Compound Interest':
        final total = CalculatorService.compoundAmount(x, y, z);
        r = 'Maturity: ${money(total)}\nInterest: ${money(total - x)}';
        break;
      case 'EMI Calculator':
        final months = z.round();
        final emi = CalculatorService.emi(x, y, months);
        r = 'Monthly EMI: ${money(emi)}\nTotal payment: ${money(emi * months)}';
        break;
      case 'Monthly Vaddi':
        final i = CalculatorService.monthlyVaddi(x, y, z);
        r = 'Interest: ${money(i)}\nTotal: ${money(x + i)}';
        break;
      case 'Discount':
        final p = CalculatorService.discountPrice(x, y);
        r = 'Final price: ${money(p)}\nYou save: ${money(x - p)}';
        break;
      case 'GST':
        final g = CalculatorService.gstAmount(x, y);
        r = 'GST: ${money(g)}\nTotal: ${money(x + g)}';
        break;
      case 'Fuel Cost':
        r = 'Estimated monthly fuel cost: ${money(CalculatorService.fuelMonthlyCost(x, y, z))}';
        break;
      default:
        r = 'Estimated electricity cost: ${money(CalculatorService.electricityCost(x, y, z, 8))}\nUsing ₹8/unit default';
    }
    setState(() => result = r);
  }
  ...
  Widget build(BuildContext context) {
    final l = labels;
    return Scaffold(
      appBar: AppBar(title: Text(widget.tool)),
      body: ListView(
        ...
        children: [
          ...
          Card(
            child: Padding(
              padding: const EdgeInsets.all(20),
              child: Text(result, style: Theme.of(context).textTheme.titleMedium),
            ),
          ),
        ],
      ),
